...
describe("Java analyzer (wired into the default indexer)", () => {
  it("indexes .java files at the deep tier", async () => {
    const { store, stats } = await createDefaultIndexer().index(root);

    expect(stats.languages).toContainEqual(
      expect.objectContaining({ language: "java", tier: "deep" }),
    );

    const sample = store.getNode(sym("Sample"));
    expect(sample?.kind).toBe("Class");
    expect(sample?.tier).toBe("deep");
